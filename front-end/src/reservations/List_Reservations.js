...
function ListReservations({ reservations }) {
	...
	async function cancelHandler({ target }) {
		...
		if (result) {
			...
			try {
				await updateTableStatus(reservation_id, 'cancelled', AbortController.signal);
			} catch (e) {
				setApiError(e);
	...
	const tableRows = reservations.map(
		({ reservation_id, first_name, last_name, mobile_number, people, status, reservation_time }) => (
			<tr key={reservation_id} className={reservationStatus(status)}>
				...
				<td>{seatButton(status, reservation_id)}</td>
				<td>{disableButtons(status, reservation_id)}</td>
			</tr>
		)
	);
